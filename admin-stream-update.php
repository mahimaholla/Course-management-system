...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if(empty(trim($_POST["c_name"]))){
        ...
        $sql = "SELECT s_id FROM stream WHERE s_id = ?";
...
if(empty(trim($_POST["sp_id"]))){
        ...
        // Prepare a select statement
        $sql = "SELECT S_name FROM stream WHERE S_name = ?";
    ...
    if(empty($c_name_err)&&empty($sp_id_err)){
        ...
        $sql = "UPDATE stream SET S_name=? where s_id=?";
        ...
        if($stmt = mysqli_prepare($link, $sql)){
           ...
           $param_sp_id = $sp_id;
            ...
            if(mysqli_stmt_execute($stmt)){
                ...
                header("location: admin-editing-streams.php");
            } else{
...
<div class="sDivider col-md-6 index0" data-colsize="6" data-title="Column" style="padding-left: 400px;width: 70%">
    ...
    <div class="wrapper">
    <form class="simblaEL" data-drag="P15" onsubmit="return simblaFormSubmit(this)" method="POST" id="P15" data-title="Form" name="" data-failure="" data-failure-type="text" data-success="" data-success-type="text"   style="width: 75%; padding-left: 90px;
